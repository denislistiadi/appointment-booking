      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {!loading
          ? data.map((item: IDoctor) => (
              <div
                key={item.id}
                className="border hover:border-primary transition-all ease-in-out cursor-pointer rounded-xl p-3"
              >
                <Image
          ...
          : Array.from(Array(8).keys()).map((item) => (
              <div key={item} className="flex flex-col border border-gray-100 rounded-lg p-3 space-y-3">
